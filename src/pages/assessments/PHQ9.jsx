...
export default function PHQ9() {
  ...
  const recommendations = {
    ...
    "Mild depression": [
      "تمرین‌های خودمراقبتی را امتحان کنید.",
      <Link to="/content" className="text-purple-600 hover:underline">مشاهده محتوای خودمراقبتی</Link>
    ],
    "Moderate depression": [
  ...
  const handleSubmit = async (e) => {
    ...
    const maxScore = PHQ9_ITEMS.length * 3 // حداکثر امتیاز: 9 سوال × 3
    const percentage = ((score / maxScore) * 100).toFixed(1)
    const interp = interpretPHQ9(score)
    const resultData = { score, percentage, interp, suicideRisk: answers[8] > 0 }
    ...
    try {
      ...
      await api.post(
        ...
        { type: "PHQ-9", score, date: new Date().toISOString() },
    ...
    <div className="max-w-3xl mx-auto">
      ...
      {result && (
        <div className="mt-6 bg-white p-5 rounded-xl shadow space-y-2">
          <div>امتیاز کل: {result.score} / {PHQ9_ITEMS.length * 3}</div>
          <div>درصد: {result.percentage}%</div>
          <div>شدت: {result.interp.level}</div>
          {result.suicideRisk && (
            <div className="text-red-600 font-bold">
              ⚠️ خطر خودکشی – لطفاً فوراً به متخصص مراجعه کنید یا با شماره ۱۱۵ تماس بگیرید
            </div>
          )}
          <div className="mt-4">
            <h4 className="font-bold text-purple-600">توصیه‌ها:</h4>
            <ul className="list-disc pr-5 space-y-1">
              {recommendations[result.interp.level].map((rec, i) => (
